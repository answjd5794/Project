   ...
   /**
	...
	private void afterLogin() {
		while (true) {
			...
			try {
				...
				case 2:
					// 장바구니 조회 메서드 호출
					bucketInput(bvo.getBook_id(), mvo.getMem_id());
					break;
	...
	private void pointCharge() {
		while (true) {
			...
			System.out.println();
			System.out.println("1. 3000포인트");
			System.out.println("2. 5000포인트");
			System.out.println("3. 10000포인트");
	...
	private void showIT() {
	      ...
	      if (showIT.isEmpty()== true) {
	    	  ...
	    	  System.out.println("주문을 원하시는 서적명을 입력해주세요.");
	    	  Scanner scan = new Scanner(System.in);
	    	  String input = scan.nextLine();
	    	  
	    	  for(int i = 0; i<showIT.size(); i++){
	    		  if(input.equals(showIT.get(i).getBook_name())){
	    			  System.out.println("장바구니로 이동합니다.");
	    			  int book_id = showIT.get(i).getBook_id();
	    			  String mem_id = mvo.getMem_id();
	    			  bucketInput(book_id, mem_id);
	    			  break;
	    		  }
	    	  }
	      }
	...
	private void bucketInput(int book_id, String mem_id) {
		int book_Id = bvo.getBook_id();
		String mem_Id = mvo.getMem_id();
